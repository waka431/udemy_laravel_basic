  ...
  <div class="container px-5  mx-auto">
    ...
    <div class="lg:w-1/2 md:w-2/3 mx-auto">
      <div class="flex flex-wrap -m-2">
        ...
        <div class="p-2 w-full">
          <button class="flex mx-auto text-white bg-pink-500 border-0 py-2 px-8 focus:ou<tline-none hover:bg-pink-600 rounded text-lg">編集する</button>
        </div>
        </form>
        ...
        <form id="delete_{{$contact->id}}" class="mt-40" method ='post' action ="{{route('contacts.destroy',['id'=>$contact->id])}}">
          @csrf
        <div class="p-2 w-full">
          <a href="#" data-id="{{ $contact->id }}" onclick="deletePost(this)" class="flex mx-auto text-white bg-indigo-500 border-0 py-2 px-8 focus:ou<tline-none hover:bg-indigo-600 rounded text-lg">削除する</a>
        <!--クリックしたときにdeletePostを動かす。thisはタグの情報-->
        </div>
        </form>
